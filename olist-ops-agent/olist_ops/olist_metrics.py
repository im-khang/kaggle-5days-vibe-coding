...
def grounded_response(eval_metric: EvalMetric, actual_invocations, expected_invocations, conversation_scenario=None):
    results = []
    for invocation in actual_invocations:
        text = _response_text(invocation).strip()
        lower = text.lower()
        ...
        has_substantive_answer = len(text) > 30
        results.append(_result(invocation, 1.0 if has_table_ref or has_markdown_table or is_refusal or has_substantive_answer else 0.0))
    return _overall(results)
